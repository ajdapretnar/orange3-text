...
"""This module implements the concept of a Dictionary -- a mapping between words
...
class Dictionary(Mapping):
    ...
    cfs : dict of (int, int)
        Collection frequencies: token_id -> how many instances of this token are
    ...
    def __getitem__(self, tokenid):
        ...
        if len(self.id2token) != len(self.token2id):
            ...
            self.id2token = revdict(self.token2id)
    ...
    def __str__(self):
        ...
        return "%s<%i unique tokens: %s%s>" % (
            self.__class__.__name__, len(self), some_keys, '...'
            if len(self) > 5 else ''
        )
    ...
    def add_documents(self, documents, prune_at=2000000):
        ...
        ----------
        documents : iterable of iterable of str
            Input corpus. All tokens should be already **tokenized and
            normalized**.
        prune_at : int, optional
            Dictionary will try to keep no more than `prune_at` words in its
            mapping, to limit its RAM
            footprint, the correctness is not guaranteed.
            Use :meth:`~orangecontrib.text.preprocess.dictionary.Dictionary.filter_extremes`
    ...
    def doc2bow(self, document, allow_update=False, return_missing=False):
        """Convert `document` into the bag-of-words (BoW) format = list of
        ...
        allow_update : bool, optional
            Update self, by adding new tokens from `document` and updating
            internal corpus statistics.
        return_missing : bool, optional
            Return missing tokens (tokens present in `document` but not in self)
            with frequencies?
        ...
        list of (int, int), dict of (str, int)
            If `return_missing` is True, return BoW representation of `document`
             + dictionary with missing
        ...
        if isinstance(document, str):
            raise TypeError("doc2bow expects an array of unicode tokens on "
                            "input, not a single string")
    ...
    def doc2idx(self, document, unknown_word_index=-1):
        ...
        of `token_id`.
        Replace all unknown words i.e, words not in the dictionary with the
        index as set via `unknown_word_index`.
        ...
        """
        if isinstance(document, str):
            raise TypeError("doc2idx expects an array of unicode tokens on "
                            "input, not a single string")

        document = [word if isinstance(word, str) else str(word, 'utf-8')
    ...
    def filter_n_most_frequent(self, remove_n):
        ...
        most_frequent_ids = sorted(most_frequent_ids, key=self.dfs.get,
                                   reverse=True)
    ...
    def filter_tokens(self, bad_ids=None, good_ids=None):
        ...
        :class:`~orangecontrib.text.preprocess.dictionary.Dictionary`.

        Alternatively, keep selected `good_ids` in
        :class:`~orangecontrib.text.preprocess.dictionary.Dictionary` and
        remove the rest.
        ...
        if bad_ids is not None:
            bad_ids = set(bad_ids)
            self.token2id = {token: tokenid for token, tokenid in
                             self.token2id.items() if tokenid not in bad_ids}
            self.cfs = {tokenid: freq for tokenid, freq in
                        self.cfs.items() if tokenid not in bad_ids}
            self.dfs = {tokenid: freq for tokenid, freq in
                        self.dfs.items() if tokenid not in bad_ids}
        if good_ids is not None:
            good_ids = set(good_ids)
            self.token2id = {token: tokenid for token, tokenid in
                             self.token2id.items() if tokenid in good_ids}
            self.cfs = {tokenid: freq for tokenid, freq in
                        self.cfs.items() if tokenid in good_ids}
            self.dfs = {tokenid: freq for tokenid, freq in
                        self.dfs.items() if tokenid in good_ids}
        self.compactify()
    ...
    def compactify(self):
        ...
        self.token2id = {token: idmap[tokenid] for token, tokenid
                         in self.token2id.items()}
        self.id2token = {}
    ...
    def patch_with_special_tokens(self, special_token_dict):
        ...
        **Usecase:** when doing sequence modeling (e.g. named entity recognition),
        one may  want to specify special tokens that behave differently than
        others. One example is the "unknown" token, and another is the
        padding token. It is usual to set the padding token to have index
        `0`, and patching the dictionary with `{'<PAD>': 0}` would be one way
        to specify this.

        Parameters
        ----------
        special_token_dict : dict of (str, int)
            dict containing the special tokens as keys and their wanted indices
            as values.
    ...
    def most_common(self, n: Optional[int] = None) -> List[Tuple[str, int]]:
        """Return a list of the n most common words and their counts from the
        ...
        n : int or None, optional
            The number of most common words to be returned. If `None`, all
        ...
        most_common : list of (str, int)
            The n most common words and their counts from the most common to
            the least.
    ...
    def from_corpus(corpus, id2word=None):
        """Create :class:`~orangecontrib.text.preprocess.dictionary.Dictionary`
        ...
        id2word : dict of (int, object)
            Mapping id -> word. If None, the mapping
            `id2word[word_id] = str(word_id)` will be used.

        Notes
        -----
        This can be useful if you only have a term-document BOW matrix
        (represented by `corpus`), but not the original text corpus. This
        method will scan the term-document count matrix for all word ids that
        appear in it, then construct
        :class:`~orangecontrib.text.preprocess.dictionary.Dictionary` which maps
        each `word_id -> id2word[word_id]`. `id2word` is an optional
        dictionary that maps the `word_id` to a token. In case `id2word`
        isn't specified the mapping `id2word[word_id] = str(word_id)` will be
        used.
        ...
        if id2word is None:
            ...
            result.token2id = {to_unicode(token): idx for idx, token in id2word.items()}
